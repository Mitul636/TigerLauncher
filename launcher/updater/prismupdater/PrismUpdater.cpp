...
#include "updater/Tigerupdater/UpdaterDialogs.h"
...
void appDebugOutput(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    TigerUpdaterApp* app = static_cast<TigerUpdaterApp*>(QCoreApplication::instance());
...
TigerUpdaterApp::TigerUpdaterApp(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    parser.setApplicationDescription(QObject::tr("An auto-updater for Tiger Launcher"));
    ...
    if (!dirParam.isEmpty()) {
        // the dir param. it makes Tiger Launcher data path point to whatever the user specified
        // on command line
    ...
    m_updateLogPath = FS::PathCombine(m_dataPath, "logs", "Tiger_launcher_update.log");
    ...
    QString Tiger_executable = FS::PathCombine(binPath, BuildConfig.LAUNCHER_APP_BINARY_NAME);
#if defined Q_OS_WIN32
    Tiger_executable.append(".exe");
#endif

    if (!QFileInfo(Tiger_executable).isFile()) {
        showFatalErrorMessage(tr("Unsupported Installation"), tr("The updater can not find the main executable."));
    }

    m_TigerExecutable = Tiger_executable;

    auto Tiger_update_url = parser.value("update-url");
    if (Tiger_update_url.isEmpty())
        Tiger_update_url = BuildConfig.UPDATER_GITHUB_REPO;

    m_TigerRepoUrl = QUrl::fromUserInput(Tiger_update_url);
    ...
    auto version = parser.value("Tiger-version");
    if (!version.isEmpty()) {
        if (version.contains('-')) {
        ...
        m_TigerVersionMajor = version_parts.takeFirst().toInt();
        m_TigerVersionMinor = version_parts.takeFirst().toInt();
        if (!version_parts.isEmpty())
            m_TigerVersionPatch = version_parts.takeFirst().toInt();
        else
            m_TigerVersionPatch = 0;
    }

    m_allowPreRelease = parser.isSet("pre-release");

    auto marker_file_path = QDir(m_rootPath).absoluteFilePath(".Tiger_launcher_updater_unpack.marker");
    ...
    if (marker_file.exists()) {
        ...
        QMetaObject::invokeMethod(this, &TigerUpdaterApp::loadReleaseList, Qt::QueuedConnection);
    }
}

TigerUpdaterApp::~TigerUpdaterApp()
{
...
void TigerUpdaterApp::fail(const QString& reason)
{
...
void TigerUpdaterApp::abort(const QString& reason)
{
    qCritical() << qPrintable(reason);
    m_status = Aborted;
    exit(2);
...
void TigerUpdaterApp::showFatalErrorMessage(const QString& title, const QString& content)
{
    m_status = Failed;
...
void TigerUpdaterApp::run()
{
    qDebug() << "found" << m_releases.length() << "releases on github";
    qDebug() << "loading exe at" << m_TigerExecutable;
    ...
    if (!loadTigerVersionFromExe(m_TigerExecutable)) {
        m_TigerVersion = BuildConfig.printableVersionString();
        m_TigerVersionMajor = BuildConfig.VERSION_MAJOR;
        m_TigerVersionMinor = BuildConfig.VERSION_MINOR;
        m_TigerVersionPatch = BuildConfig.VERSION_PATCH;
        m_prsimVersionChannel = BuildConfig.VERSION_CHANNEL;
        m_TigerGitCommit = BuildConfig.GIT_COMMIT;
    }
    m_status = Succeeded;

    qDebug() << "Executable reports as:" << m_TigerBinaryName << "version:" << m_TigerVersion;
    qDebug() << "Version major:" << m_TigerVersionMajor;
    qDebug() << "Version minor:" << m_TigerVersionMinor;
    qDebug() << "Version minor:" << m_TigerVersionPatch;
    qDebug() << "Version channel:" << m_prsimVersionChannel;
    qDebug() << "Git Commit:" << m_TigerGitCommit;
    ...
    if (m_isFlatpak) {
        showFatalErrorMessage(tr("Updating flatpack not supported"), tr("Actions outside of checking if an update is available are not "
                                                                        "supported when running the flatpak version of Tiger Launcher."));
        return;
    }
    if (m_isAppimage) {
        bool result = true;
...
void TigerUpdaterApp::moveAndFinishUpdate(QDir target)
{
    ...
    if (error) {
        ...
        auto fail_marker = FS::PathCombine(m_dataPath, ".Tiger_launcher_update.fail");
        FS::copy(m_updateLogPath, fail_marker).overwrite(true)();
    } else {
        logUpdate(tr("Update succeed."));
        auto success_marker = FS::PathCombine(m_dataPath, ".Tiger_launcher_update.success");
        FS::copy(m_updateLogPath, success_marker).overwrite(true)();
    }
    auto update_lock_path = FS::PathCombine(m_dataPath, ".Tiger_launcher_update.lock");
...
void TigerUpdaterApp::printReleases()
{
...
QList<GitHubRelease> TigerUpdaterApp::nonDraftReleases()
{
...
QList<GitHubRelease> TigerUpdaterApp::newerReleases()
{
    QList<GitHubRelease> newer;
    for (auto rls : nonDraftReleases()) {
        if (rls.version > m_TigerVersion)
            newer.append(rls);
    }
    return newer;
}

GitHubRelease TigerUpdaterApp::selectRelease()
{
    ...
    SelectReleaseDialog dlg(Version(m_TigerVersion), releases);
...
QList<GitHubReleaseAsset> TigerUpdaterApp::validReleaseArtifacts(const GitHubRelease& release)
{
...
GitHubReleaseAsset TigerUpdaterApp::selectAsset(const QList<GitHubReleaseAsset>& assets)
{
...
void TigerUpdaterApp::performUpdate(const GitHubRelease& release)
{
...
QFileInfo TigerUpdaterApp::downloadAsset(const GitHubReleaseAsset& asset)
{
...
bool TigerUpdaterApp::callAppImageUpdate()
{
...
void TigerUpdaterApp::clearUpdateLog()
{
    FS::deletePath(m_updateLogPath);
}

void TigerUpdaterApp::logUpdate(const QString& msg)
{
...
void TigerUpdaterApp::performInstall(QFileInfo file)
{
    ...
    if (update_lock.exists()) {
        ...
        auto infoMsg =
            tr("This installation has a update lock file present at: %1\n"
               ...
               "proceeding.\n"
               "Check the Tiger Launcher updater log at: \n"
               "%7\n"
    ...
    auto changelog_path = FS::PathCombine(m_dataPath, ".Tiger_launcher_update.changelog");
    FS::write(changelog_path, m_install_release.body.toUtf8());

    logUpdate(tr("Updating from %1 to %2").arg(m_TigerVersion).arg(m_install_release.tag_name));
    if (m_isPortable || file.fileName().endsWith(".zip") || file.fileName().endsWith(".tar.gz")) {
        write_lock_file(update_lock_path, QDateTime::currentDateTime(), m_TigerVersion, m_install_release.tag_name, m_rootPath, m_dataPath);
...
void TigerUpdaterApp::unpackAndInstall(QFileInfo archive)
{
    logUpdate(tr("Backing up install"));
    backupAppDir();

    if (auto loc = unpackArchive(archive)) {
        auto marker_file_path = loc.value().absoluteFilePath(".Tiger_launcher_updater_unpack.marker");
...
void TigerUpdaterApp::backupAppDir()
{
    auto manifest_path = FS::PathCombine(m_rootPath, "manifest.txt");
    QFileInfo manifest(manifest_path);
    ...
    if (file_list.isEmpty()) {
        ...
        if (BuildConfig.BUILD_ARTIFACT.toLower().contains("linux")) {
            file_list.append({ "TigerLauncher", "bin", "share", "lib" });
        } else {  // windows by process of elimination
            file_list.append({
                "jars",
                "TigerLauncher.exe",
                "TigerLauncher_filelink.exe",
                "TigerLauncher_updater.exe",
                "qtlogging.ini",
                "imageformats",
                "iconengines",
                "platforms",
    ...
    auto backup_dir =
        FS::PathCombine(app_dir.absolutePath(),
                        QStringLiteral("backup_") + QString(m_TigerVersion).replace(s_replaceRegex, QString("_")) + "-" + m_TigerGitCommit);
    FS::ensureFolderPathExists(backup_dir);
    auto backup_marker_path = FS::PathCombine(m_dataPath, ".Tiger_launcher_update_backup_path.txt");
...
std::optional<QDir> TigerUpdaterApp::unpackArchive(QFileInfo archive)
{
    auto temp_extract_path = FS::PathCombine(m_dataPath, "Tiger_launcher_update_release");
...
bool TigerUpdaterApp::loadTigerVersionFromExe(const QString& exe_path)
{
    QProcess proc = QProcess();
    ...
    if (first_parts.length() < 2)
        return false;
    m_TigerBinaryName = first_parts.takeFirst();
    auto version = first_parts.takeFirst().trimmed();
    m_TigerVersion = version;
    if (version.contains('-')) {
        auto index = version.indexOf('-');
        m_prsimVersionChannel = version.mid(index + 1);
        version = version.left(index);
    ...
    if (!version_parts.isEmpty())
        ...
        m_TigerVersionPatch = 0;
    m_TigerGitCommit = lines.takeFirst().simplified();
    return true;
}

void TigerUpdaterApp::loadReleaseList()
{
    auto github_repo = m_TigerRepoUrl;
...
void TigerUpdaterApp::downloadReleasePage(const QString& api_url, int page)
{
    int per_page = 30;
    ...
    connect(download.get(), &Net::Download::failed, this, &TigerUpdaterApp::downloadError);
...
int TigerUpdaterApp::parseReleasePage(const QByteArray* response)
{
...
GitHubRelease TigerUpdaterApp::getLatestRelease()
{
...
bool TigerUpdaterApp::needUpdate(const GitHubRelease& release)
{
    auto current_ver = Version(QString("%1.%2.%3").arg(m_TigerVersionMajor).arg(m_TigerVersionMinor).arg(m_TigerVersionPatch));
    return current_ver < release.version;
}

void TigerUpdaterApp::downloadError(QString reason)
{
