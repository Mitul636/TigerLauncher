...
void V1::updateModIndex(const QDir& index_dir, Mod& mod)
{
    ...
    QTextStream in_stream(&index_file);
    {
        auto tbl = toml::table{ { "name", mod.name.toStdString() },
                                ...
                                { "x-TigerLauncher-loaders", loaders },
                                { "x-TigerLauncher-mc-versions", mcVersions },
                                { "x-TigerLauncher-release-type", mod.releaseType.toString().toStdString() },
                                { "x-TigerLauncher-version-number", mod.version_number.toStdString() },
                                { "x-TigerLauncher-dependencies", deps },
                                { "download",
                                  toml::table{
...
auto V1::getIndexForMod(const QDir& index_dir, QString slug) -> Mod
{
    ...
    {  // Basic info
        ...
        mod.releaseType = ModPlatform::IndexedVersionType::fromString(table["x-TigerLauncher-release-type"].value_or(""));
        if (auto loaders = table["x-TigerLauncher-loaders"]; loaders && loaders.is_array()) {
        ...
        if (auto versions = table["x-TigerLauncher-mc-versions"]; versions && versions.is_array()) {
    ...
    mod.version_number = table["x-TigerLauncher-version-number"].value_or("");
    ...
    {  // dependencies
        auto deps = table["x-TigerLauncher-dependencies"].as_array();
        if (deps) {
